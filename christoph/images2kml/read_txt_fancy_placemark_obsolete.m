...
for i=1:nc1
    ...
    % the highlight placemark (i.e. if you put the cursor on top) is the
    % image taken by the smartphone. Notice that you need to scale it:
    fprintf(fidw,'      <Style id="highlightPlacemark">\n');
    fprintf(fidw,'        <IconStyle>\n');
    fprintf(fidw,'          <Icon>\n');

    if (C{15}(i) > -8.0) % test gravitational vector
        fprintf(fidw,'        <href>%s%s</href>\n',dirdir,C{1}(i,:));
    else
        fprintf(fidw,'        <href>%sflip/%s</href>\n',dirdir,C{1}(i,:));
    end
   
    fprintf(fidw,'          </Icon>\n');
    fprintf(fidw,'          <scale>10.0</scale>\n');
    fprintf(fidw,'        </IconStyle>\n');
    fprintf(fidw,'      </Style>\n');

    fprintf(fidw,'    <StyleMap id="exampleStyleMap">\n');
    fprintf(fidw,'      <Pair>\n');
    fprintf(fidw,'        <key>normal</key>\n');
    fprintf(fidw,'        <styleUrl>#normalPlacemark</styleUrl>\n');
    fprintf(fidw,'      </Pair>\n');
    fprintf(fidw,'      <Pair>\n');
    fprintf(fidw,'        <key>highlight</key>\n');
    fprintf(fidw,'        <styleUrl>#highlightPlacemark</styleUrl>\n');
    fprintf(fidw,'      </Pair>\n');
    fprintf(fidw,'    </StyleMap>\n');
    
    
    fprintf(fidw,'  <Placemark>\n');
    %fprintf(fidw,'    <name>Simple placemark</name>\n');
    fprintf(fidw,'      <styleUrl>#exampleStyleMap</styleUrl>\n');
