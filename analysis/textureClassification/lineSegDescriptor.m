function features = lineSegDescriptor(im, pixels)
%TODO: Explain more and paste into html
% features(nseg, nf)
...
for s = 1:nseg    
    ...
    f = 0;
    for k = 1:3
        features(s, f+k) = mean(im(pixels(s).PixelIdxList+(k-1)*imw*imh));
    end
    f = f + 3;
    % hsv means
    ...
    f = f + 3;
    % hue histogram    
    features(s, f+[1:5]) = hist(hhist(pixels(s).PixelIdxList), [1:5])+1;
    features(s, f+[1:5]) = features(s, f+[1:5]) / sum(features(s, f+[1:5]));
    f = f + 5;
    % sat histogram        
    features(s, f+[1:3]) = hist(shist(pixels(s).PixelIdxList), [1:3])+1;
    features(s, f+[1:3]) = features(s, f+[1:3]) / sum(features(s, f+[1:3]));
    f = f + 3;
    % texture means
    for k = 1:ntext
        features(s, f+k) = mean(imtext(pixels(s).PixelIdxList+(k-1)*npix));
    end
    f = f + ntext;
    % texture histogram
    features(s, f+(1:ntext)) = hist(texthist(pixels(s).PixelIdxList), (1:ntext))+1;
    features(s, f+(1:ntext)) = features(s, f+(1:ntext)) / sum(features(s, f+(1:ntext)));
    f = f + ntext;
    % location
    xvals = xim(pixels(s).PixelIdxList);
    yvals = yim(pixels(s).PixelIdxList);
    ...
    features(s, f+8) = size(pixels(s).PixelIdxList,1)/imh/imw;
end
